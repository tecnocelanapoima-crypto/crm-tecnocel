...
def crear():
    if request.method == 'POST':
        ...
        db = get_db()
        db.execute('''
            INSERT INTO productos (nombre, marca, categoria, precio, stock, descripcion, foto)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (nombre, marca, categoria, float(precio), int(stock), descripcion, foto_filename))
        db.commit()
        db.close()
...
def editar(id):
    ...
    if request.method == 'POST':
        ...
        if eliminar_foto and foto_filename:
            old_path = os.path.join(current_app.root_path, 'static', 'uploads', 'productos', foto_filename)
        ...
        db.execute('''
            ...
            WHERE id=?
        ''', (nombre, marca, categoria, float(precio), int(stock), descripcion, foto_filename, id))
        db.commit()
        db.close()
...
def eliminar(id):
    db = get_db()
    producto = db.execute('SELECT foto FROM productos WHERE id = ?', (id,)).fetchone()
    if producto:
        if producto['foto']:
            old_path = os.path.join(current_app.root_path, 'static', 'uploads', 'productos', producto['foto'])
            if os.path.exists(old_path):
                os.remove(old_path)
